...
must_haves:
  truths:
    - "Changed members are detected and flagged for sync"
    - "Unchanged members are skipped (no unnecessary API calls)"
    - "Sync state persists across script runs"
    - "Deleted members can be identified by comparing against current list"
  artifacts:
