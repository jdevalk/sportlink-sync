...
- If `errors` is not an array and not a positive number, return immediately (no-op)
- If `errors` is a number > 0 (like nikki's `stats.rondoClub.errors` which is a count), record a single error with message `"Step reported ${errors} error(s)"`
- If `errors` is an array, iterate and for each error object extract:
  - **memberIdentifier** (precedence order, first non-falsy wins): `error.knvb_id` → `error.email` → `error.dossier_id` → `error.team_name` → `error.commissie_name` → `null`
...
**club_slug handling:** All three tables have `club_slug TEXT NOT NULL DEFAULT 'rondo'`. The INSERT statements intentionally OMIT `club_slug` so the DEFAULT applies. This is correct for the current single-club setup. Phase 39 (Multi-Club Readiness) will add club_slug as a constructor parameter when a second club is onboarded. No premature multi-club logic.
...
- Step 'sportlink-download': after downloadResult check, endStep with created=stats.downloaded. No errors array for this step (download failure causes early return).
- Step 'laposta-prepare': after prepareResult check, endStep with created=stats.prepared. No step-specific errors (prepare failure causes early return).
- Step 'laposta-submit': after submitResult, endStep with created=stats.added, updated=stats.updated, failed=stats.errors.length. Call `tracker.recordErrors('laposta-submit', stepId, stats.errors)` — note: `stats.errors` is populated from `stats.lists[].errors` at lines 203-207, which ARE the laposta submit errors (not a general aggregate).
- Step 'rondo-club-sync': endStep with created=stats.rondoClub.created, updated=stats.rondoClub.updated, skipped=stats.rondoClub.skipped, failed=stats.rondoClub.errors.length. Call `tracker.recordErrors('rondo-club-sync', stepId, stats.rondoClub.errors)`.
- Step 'photo-download': endStep with created=stats.photos.downloaded, failed=stats.photos.errors.length. Call `tracker.recordErrors('photo-download', stepId, stats.photos.errors)`.
- Step 'photo-upload': endStep with created=stats.photos.uploaded, skipped=stats.photos.skipped, failed=stats.photos.errors.length. Call `tracker.recordErrors('photo-upload', stepId, stats.photos.errors)`.
- Step 'reverse-sync': endStep with updated=stats.reverseSync.synced, failed=stats.reverseSync.errors.length. Call `tracker.recordErrors('reverse-sync', stepId, stats.reverseSync.errors)`.
...
- Step 'nikki-download': endStep with created=stats.download.count, failed=stats.download.errors.length. Call `tracker.recordErrors('nikki-download', stepId, stats.download.errors)`.
- Step 'rondo-club-sync': endStep with updated=stats.rondoClub.updated, skipped=stats.rondoClub.skipped, failed=(typeof stats.rondoClub.errors === 'number' ? stats.rondoClub.errors : 0). Call `tracker.recordErrors('rondo-club-sync', stepId, stats.rondoClub.errors)` — `recordErrors` handles numbers by recording a single error with the count.

**sync-teams.js** (pipeline: 'teams', 3 steps):
- Step 'team-download': endStep with created=stats.download.teamCount, failed=stats.download.errors.length. Call `tracker.recordErrors('team-download', stepId, stats.download.errors)`.
- Step 'team-sync': endStep with created=stats.teams.created, updated=stats.teams.updated, skipped=stats.teams.skipped, failed=stats.teams.errors.length. Call `tracker.recordErrors('team-sync', stepId, stats.teams.errors)`.
- Step 'work-history-sync': endStep with created=stats.workHistory.created, updated=stats.workHistory.ended, skipped=stats.workHistory.skipped, failed=stats.workHistory.errors.length. Call `tracker.recordErrors('work-history-sync', stepId, stats.workHistory.errors)`.

**sync-functions.js** (pipeline: 'functions', 3 steps):
- Step 'functions-download': endStep with created=stats.download.total, failed=stats.download.errors.length. Call `tracker.recordErrors('functions-download', stepId, stats.download.errors)`.
- Step 'commissie-sync': endStep with created=stats.commissies.created, updated=stats.commissies.updated, skipped=stats.commissies.skipped, failed=stats.commissies.errors.length. Call `tracker.recordErrors('commissie-sync', stepId, stats.commissies.errors)`.
- Step 'commissie-work-history-sync': endStep with created=stats.workHistory.created, updated=stats.workHistory.ended, skipped=stats.workHistory.skipped, failed=stats.workHistory.errors.length. Call `tracker.recordErrors('commissie-work-history-sync', stepId, stats.workHistory.errors)`.

**sync-discipline.js** (pipeline: 'discipline', 2 steps):
- Step 'discipline-download': endStep with created=stats.download.caseCount, failed=stats.download.errors.length. Call `tracker.recordErrors('discipline-download', stepId, stats.download.errors)`.
- Step 'discipline-sync': endStep with created=stats.sync.created, updated=stats.sync.updated, skipped=stats.sync.skipped, failed=stats.sync.errors.length. Call `tracker.recordErrors('discipline-sync', stepId, stats.sync.errors)`.

**sync-freescout.js** (pipeline: 'freescout', 1 step):
- Step 'freescout-sync': endStep with created=stats.created, updated=stats.updated, skipped=stats.skipped, failed=stats.errors.length. Call `tracker.recordErrors('freescout-sync', stepId, stats.errors)`.
