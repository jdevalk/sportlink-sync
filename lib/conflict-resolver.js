...
const { logConflictResolution } = require('./stadion-db');

/**
 * Resolve field-level conflicts between Sportlink and Stadion data.
 * Uses last-write-wins (LWW) logic with 5-second grace period.
 *
 * @param {Object} member - Member row from stadion_members (includes timestamp columns)
 * @param {Object} sportlinkData - Current data from Sportlink
 * @param {Object} stadionData - Current data from Stadion
 ...
 */
function resolveFieldConflicts(member, sportlinkData, stadionData, db, logger = null) {
  ...
  for (const field of TRACKED_FIELDS) {
    ...
    const stadionValue = stadionData[field];
    ...
    // Case 3: Only Stadion has timestamp
    if (!sportlinkTs && stadionTs) {
      resolutions.set(field, {
        value: stadionValue,
        winner: 'stadion',
    ...
    if (sportlinkValue === stadionValue) {
    ...
    const winningValue = winner === 'stadion' ? stadionValue : sportlinkValue;
    const reason = comparison > 0 ? 'stadion_newer' : 'sportlink_newer';
    ...
    const conflictRecord = {
      ...
      stadion_value: String(stadionValue || ''),
      sportlink_modified: sportlinkTs,
      stadion_modified: stadionTs,
...
if (require.main === module) {
  ...
  const member1 = {
    ...
    email_stadion_modified: null,
    email2_sportlink_modified: null,
    email2_stadion_modified: '2026-01-29T10:00:00.000Z',
    mobile_sportlink_modified: '2026-01-29T10:00:00.000Z',
    mobile_stadion_modified: null,
    phone_sportlink_modified: null,
    phone_stadion_modified: null,
    datum_vog_sportlink_modified: null,
    datum_vog_stadion_modified: null,
    freescout_id_sportlink_modified: null,
    freescout_id_stadion_modified: null,
    financiele_blokkade_sportlink_modified: null,
    financiele_blokkade_stadion_modified: null
  };
  const sportlink1 = { email: 'sportlink@example.com', email2: 'sportlink2@example.com', mobile: '0612345678' };
  const stadion1 = { email: 'stadion@example.com', email2: 'stadion2@example.com', mobile: '0687654321' };

  const result1 = resolveFieldConflicts(member1, sportlink1, stadion1, db);
  console.log('  email (both NULL):', result1.resolutions.get('email').winner, '=', result1.resolutions.get('email').value);
  console.log('  email2 (only Stadion has timestamp):', result1.resolutions.get('email2').winner, '=', result1.resolutions.get('email2').value);
  ...
  const member2 = {
    ...
    email_stadion_modified: '2026-01-29T10:00:03.000Z', // 3 seconds apart (within tolerance)
    email2_sportlink_modified: null,
    email2_stadion_modified: null,
  ...
  const stadion2 = { email: 'stadion@example.com' };

  const result2 = resolveFieldConflicts(member2, sportlink2, stadion2, db);
  ...
  const member3 = {
    knvb_id: 'TEST003',
    email_sportlink_modified: '2026-01-29T10:00:00.000Z',
    email_stadion_modified: '2026-01-29T10:10:00.000Z', // 10 minutes apart
    email2_sportlink_modified: null,
    email2_stadion_modified: null,
    mobile_sportlink_modified: null,
    mobile_stadion_modified: null,
  ...
  const stadion3 = { email: 'stadion@example.com' };

  const result3 = resolveFieldConflicts(member3, sportlink3, stadion3, db);
  console.log('  email (Stadion 10min newer):', result3.resolutions.get('email').winner, '=', result3.resolutions.get('email').value);
