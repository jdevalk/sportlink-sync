/**
 * Stadion Change Detection Module
 * Detects changes in Stadion that need reverse sync to Sportlink.
 */

const { stadionRequest } = require('./stadion-client');
const { openDb, logChangeDetection, getLastDetectionTime, updateLastDetectionTime } = require('./stadion-db');
...
/**
 * Extract field value from Stadion member data.
 * Handles different field types (contact_info repeater vs direct ACF fields).
 *
 * @param {Object} stadionData - Stadion member data with ACF fields
...
function computeTrackedFieldsHash(knvbId, stadionData) {
  ...
  for (const field of TRACKED_FIELDS) {
    trackedValues[field] = extractFieldValue(stadionData, field);
  }
...
/**
 * Fetch modified members from Stadion API since a timestamp.
...
/**
 * Detect changes in Stadion members that need reverse sync.
...
async function detectChanges(options = {}) {
  ...
  try {
    ...
    logger.log('Fetching modified members from Stadion...');
    ...
    for (const member of modifiedMembers) {
      ...
      for (const field of TRACKED_FIELDS) {
        ...
        const change = {
          ...
          stadion_modified_gmt: member.modified_gmt,
