 ...
 * Idempotent: skips if stadion_members table does not exist.
 */
function migrateStadionToRondoClub(db) {
  ...
  // Get current schema for stadion_members (includes all historical ALTER TABLE additions)
  const currentMemberSchema = db.prepare(
    "SELECT sql FROM sqlite_master WHERE type='table' AND name='stadion_members'"
  ).get()?.sql || '';
  ...
  db.transaction(() => {
    // 1. MIGRATE stadion_members to rondo_club_members
    db.exec(`
    ...
    // 2. MIGRATE stadion_parents to rondo_club_parents
    db.exec(`
    ...
    // 3. MIGRATE stadion_important_dates to rondo_club_important_dates
    db.exec(`
    ...
    // 5. MIGRATE stadion_work_history to rondo_club_work_history
    db.exec(`
    ...
    // 6. MIGRATE stadion_commissies to rondo_club_commissies
    db.exec(`
    ...
    // 7. MIGRATE stadion_commissie_work_history to rondo_club_commissie_work_history
    db.exec(`
    ...
    // 8. MIGRATE stadion_change_detections to rondo_club_change_detections
    db.exec(`
...
function getMembersNotInList(db, knvbIds) {
  if (!knvbIds || knvbIds.length === 0) {
    ...
    const stmt = db.prepare(`
      SELECT knvb_id, email, stadion_id, data_json
      FROM stadion_members
      WHERE data_json != '{}'
      ORDER BY knvb_id ASC
    `);
 ...
 * @param {Object} db - SQLite database connection
 * @returns {Array<{knvb_id: string, stadion_id: number}>}
 */
...
function getMembersNeedingPhotoDownloadKnvbIds(db) {
  const stmt = db.prepare(`
    SELECT knvb_id FROM stadion_members WHERE photo_state = 'pending_download'
  `);
