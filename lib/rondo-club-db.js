...
function migrateStadionToRondoClub(db) {
  // Idempotency check: if rondo_club_members doesn't exist, migration already done
  const tableExists = db.prepare(
    "SELECT name FROM sqlite_master WHERE type='table' AND name='rondo_club_members'"
  ).get();
  ...
  const memberColumnList = memberColumns.map(col => {
    if (col.name === 'rondo_club_id') return 'rondo_club_id as rondo_club_id';
  ...
  const memberTargetColumns = memberColumns.map(col => {
    if (col.name === 'rondo_club_id') return 'rondo_club_id';
    if (col.name.endsWith('_stadion_modified')) {
  ...
  db.transaction(() => {
    ...
    db.exec(`INSERT INTO rondo_club_members (${memberTargetColumns}) SELECT ${memberColumnList} FROM rondo_club_members`);
    ...
    db.exec(`
      ...
      SELECT id, email, rondo_club_id, data_json, source_hash, last_seen_at, last_synced_at, last_synced_hash, created_at
      FROM rondo_club_parents
    `);
    db.exec('DROP TABLE rondo_club_parents');
    ...
    db.exec(`
      ...
      SELECT id, knvb_id, date_type, date_value, rondo_club_date_id, source_hash, last_synced_hash, last_synced_at, created_at
      FROM rondo_club_important_dates
    `);
    db.exec('DROP TABLE rondo_club_important_dates');
    ...
    // 4. MIGRATE rondo_club_teams to rondo_club_teams
    const teamColumns = db.prepare('PRAGMA table_info(rondo_club_teams)').all();
    const teamColumnNames = teamColumns.map(col => col.name === 'rondo_club_id' ? 'rondo_club_id as rondo_club_id' : col.name).join(', ');
    const teamTargetNames = teamColumns.map(col => col.name === 'rondo_club_id' ? 'rondo_club_id' : col.name).join(', ');
    ...
    db.exec(`INSERT INTO rondo_club_teams (${teamTargetNames}) SELECT ${teamColumnNames} FROM rondo_club_teams`);
    db.exec('DROP TABLE rondo_club_teams');
    ...
    db.exec(`
      ...
      SELECT id, knvb_id, team_name, rondo_club_work_history_id, is_backfill, source_hash, last_synced_hash, last_synced_at, created_at
      FROM rondo_club_work_history
    `);
    db.exec('DROP TABLE rondo_club_work_history');
    ...
    db.exec(`
      ...
      SELECT id, commissie_name, sportlink_id, rondo_club_id, source_hash, last_seen_at, last_synced_at, last_synced_hash, created_at
      FROM rondo_club_commissies
    `);
    db.exec('DROP TABLE rondo_club_commissies');
    ...
    db.exec(`
      ...
      SELECT id, knvb_id, commissie_name, role_name, rondo_club_work_history_id, is_backfill, source_hash, last_synced_hash, last_synced_at, created_at
      FROM rondo_club_commissie_work_history
    `);
    db.exec('DROP TABLE rondo_club_commissie_work_history');
    ...
    db.exec(`
      ...
      SELECT id, knvb_id, field_name, old_value, new_value, detected_at, rondo_club_modified_gmt, detection_run_id, synced_at
      FROM rondo_club_change_detections
    `);
    db.exec('DROP TABLE rondo_club_change_detections');
    ...
    if (conflictColumns.some(col => col.name === 'stadion_value')) {
      ...
      db.exec(`
        ...
        SELECT id, knvb_id, field_name, sportlink_value, rondo_club_value, sportlink_modified, rondo_club_modified,
               CASE winning_system
...
function getAllTeams(db) {
  const stmt = db.prepare(`
    SELECT team_name, team_code, rondo_club_id
    FROM rondo_club_teams
