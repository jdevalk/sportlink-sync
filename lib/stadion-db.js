...
function initDb(db) {
  ...
  // huidig_vrijwilliger status from Stadion (Quick 016)
    ...
    ON CONFLICT(knvb_id) DO UPDATE SET
      ...
      photo_state = CASE
        ...
        WHEN excluded.photo_url IS NULL
          ...
          THEN 'pending_download'
        -- Photo removed (had photo_url, now doesn't)
        WHEN excluded.photo_url IS NULL
             AND stadion_members.photo_url IS NOT NULL
          THEN 'pending_delete'
        -- Photo removed (fallback: had person_image_date, now doesn't)
        WHEN excluded.photo_url IS NULL
 ...
 * Stores WordPress post ID for future updates/deletes.
 */
function updateTeamSyncState(db, sportlinkId, sourceHash, stadionId) {
  const now = new Date().toISOString();
  const stmt = db.prepare(`
    ...
    WHERE sportlink_id = ?
  `);
  stmt.run(now, sourceHash, stadionId || null, sportlinkId);
}
...
/**
 * Update sync state after successful commissie sync to Stadion.
 */
function updateCommissieSyncState(db, commissieName, sourceHash, stadionId) {
  const now = new Date().toISOString();
  const stmt = db.prepare(`
  ...
  `);
  stmt.run(now, sourceHash, stadionId || null, commissieName);
}
 ...
 * @param {string} resolution.stadion_value - Value from Stadion
 * @param {string} resolution.sportlink_modified - Sportlink timestamp
 * @param {string} resolution.stadion_modified - Stadion timestamp
 ...
 * @param {string} detection.stadion_modified_gmt - Stadion modification timestamp
 ...
 * Returns changes detected in Stadion that haven't been pushed to Sportlink yet.
 * @param {Object} db - Database connection
