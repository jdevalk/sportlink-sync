 ...
 */
function computeMemberFreeFieldsHash(knvbId, freescoutId, vogDatum) {
  const payload = stableStringify({
    knvb_id: knvbId,
 ...
 * Each record: { knvb_id, freescout_id, vog_datum }
 */
 ...
 * @param {Object} db - Database connection
 * @param {string} knvbId - Member KNVB ID
 * @returns {{freescout_id: number|null, vog_datum: string|null}|null}
 */
function getMemberFreeFieldsByKnvbId(db, knvbId) {
  const stmt = db.prepare(`
    SELECT freescout_id, vog_datum
 ...
 * @param {Object} db - Database connection
 * @returns {Array<{knvb_id: string, freescout_id: number|null, vog_datum: string|null}>}
 */
function getAllMemberFreeFields(db) {
  const stmt = db.prepare(`
    SELECT knvb_id, freescout_id, vog_datum
    FROM sportlink_member_free_fields
    ORDER BY knvb_id ASC
  `);
  return stmt.all();
