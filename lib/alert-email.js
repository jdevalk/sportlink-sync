...
async function checkAndAlertOverdue() {
  ...
  try {
    ...
    // Check each pipeline for overdue status
    for (const [name, config] of Object.entries(PIPELINE_CONFIG)) {
      ...
      if (run && run.started_at) {
        ...
        hoursSince = (now - lastRun) / (1000 * 60 * 60);
        isOverdue = hoursSince > config.hours;
      } else {
        // Never run = overdue
        isOverdue = true;
        hoursSince = 9999; // Large number for display
      }
