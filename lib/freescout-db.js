...
const DEFAULT_DB_PATH = path.join(process.cwd(), 'data', 'freescout-sync.sqlite');
...
function initDb(db) {
  ...
  // Migration: add rondo_club_synced_at column
  const cols = db.prepare('PRAGMA table_info(freescout_customers)').all();
  if (!cols.some(c => c.name === 'rondo_club_synced_at')) {
    db.exec('ALTER TABLE freescout_customers ADD COLUMN rondo_club_synced_at TEXT');
  }
}
...
function updateSyncState(db, knvbId, sourceHash, freescoutId) {
  ...
  const stmt = db.prepare(`
    ...
    SET last_synced_at = ?,
        last_synced_hash = ?,
        freescout_id = ?,
        rondo_club_synced_at = CASE
          WHEN freescout_id IS NOT NULL AND freescout_id != ? THEN NULL
          ELSE rondo_club_synced_at
        END
    WHERE knvb_id = ?
  `);
  stmt.run(now, sourceHash, freescoutId || null, freescoutId || null, knvbId);
}
...
/**
 * Get customers whose freescout_id hasn't been synced back to Rondo Club yet.
 * Returns rows where freescout_id exists but rondo_club_synced_at is NULL.
 * Returns: [{ knvb_id, freescout_id }]
 */
function getCustomersNeedingRondoClubSync(db) {
  const stmt = db.prepare(`
    SELECT knvb_id, freescout_id
    FROM freescout_customers
    WHERE freescout_id IS NOT NULL AND rondo_club_synced_at IS NULL
    ORDER BY knvb_id ASC
  `);
  return stmt.all();
}
...
/**
 * Mark a customer's freescout_id as synced to Rondo Club.
 */
function updateRondoClubSyncState(db, knvbId) {
  const now = new Date().toISOString();
  const stmt = db.prepare(`
    UPDATE freescout_customers
    SET rondo_club_synced_at = ?
    WHERE knvb_id = ?
  `);
  stmt.run(now, knvbId);
}
...
module.exports = {
  ...
  getCustomersNeedingRondoClubSync,
  updateRondoClubSyncState
};
