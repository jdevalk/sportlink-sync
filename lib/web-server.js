...
const { getPipelineOverview, getRunHistory, getRunDetail, getErrors, getRunErrors, isPipelineRunning, closeDb } = require('./dashboard-queries');
...
async function buildServer() {
  ...
  const KNOWN_PIPELINES = {
    ...
    freescout: 'FreeScout',
    teams: 'Teams',
    functions: 'Commissies (recent)',
    'functions-full': 'Functions (full)',
  ...
  app.post('/api/pipeline/:name/start', { preHandler: requireAuth }, async (request, reply) => {
    ...
    try {
      ...
      app.log.info(`Pipeline "${pipelineName}" started by ${request.session.user} (pid ${child.pid})`);
