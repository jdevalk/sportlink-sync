...
class RunTracker {
  ...
  /**
   * Delete runs, steps, and errors older than 3 days.
   * @private
   */
  _cleanup() {
    if (!this.db) return;
    ...
    try {
      ...
      this.db.prepare('DELETE FROM run_errors WHERE run_id IN (SELECT id FROM runs WHERE started_at < ?)').run(cutoff);
      this.db.prepare('DELETE FROM run_steps WHERE run_id IN (SELECT id FROM runs WHERE started_at < ?)').run(cutoff);
      this.db.prepare('DELETE FROM runs WHERE started_at < ?').run(cutoff);
    } catch (err) {
      console.error(`[run-tracker] Cleanup failed: ${err.message}`);
    }
  }
